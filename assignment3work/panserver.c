...
char buf[512];
int MODE = URGENT;
char *byebye="The session is Over\n";
char *flooropen=">\b";
char *alive="\b";
...
SUShandle (int sig)
{
    ...
    struct sockaddr_in from;
    int fromlen;
	sprintf(MSGbuffer,"%s", alive);
	sprintf(MSGbuffer,"%s", flooropen);
...
void ACTServ() {
    ...
    for(;;) {
        fromlen = sizeof(from);
        if (  (psd  = accept(ACTsd, (SA *)&from, &fromlen)) == -1) {
            if (errno == EINTR) continue;
        }
        if( (rc=recv(psd, username, sizeof(username), 0)) < 0)
            perror("receiving username  message");
        ...
        ACTconnfd[index] = psd;
        fromlen = sizeof (ACTList[index]);
        getpeername(psd,(SA *)&ACTList[index],&fromlen);
        sprintf(ACTname[index].logname,"%s", username);
    ...
    for (i = 0; i < MAXACT; i++) {
        if (ACTconnfd[i] != -1) {
            if (send (ACTconnfd[i], MSGbuffer, strlen(MSGbuffer), 0) < 0) {
                ...
                ACTconnfd[i] = -1;
                // perror ("sending");
               //  printf ("  PANELIST Client %d  Disconnected..\n", i);
            }
        }
    }
    ...
    for (i = 0; i < MAXAUD; i++) {
        if (AUDconnfd[i] != -1) {
            if (send (AUDconnfd[i], MSGbuffer, strlen(MSGbuffer), 0) < 0) {
                ...
                AUDconnfd[i] = -1;
                //perror ("sending");
              //  printf ("  AUDIENCE Client %d  Disconnected..\n", i);
            }
        }
    }
}
